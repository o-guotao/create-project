    ...
    return (
        <div className="px-2">
            <svg viewBox="0 0 64 30" fill="#1772F6" width="64" height="30" className="css-1hlrcxk">
                ...
                <path d="M42.614 8.873a2.304 2.304 0 0 0-1.508-.926 2.334 2.334 0 0 0-1.727.405l-.376.272 4.255 5.85 2.24-1.62-2.884-3.98zM57.35 8.68l-3.125 4.097 2.24 1.663 4.517-5.927-.375-.277a2.32 2.32 0 0 0-1.722-.452 2.327 2.327 0 0 0-1.536.896z"></path></svg>
        </div>
...
const Menu = () => {
    const naviagtion = useLocation()
    const getParentPath = () => {
        let pathRes;
        router.forEach(item => {
            (item.children || []).forEach((subItem) => {
                if (subItem.path && naviagtion.pathname.includes(subItem.path)) {
                    pathRes = item.path
                }
            })
        })
                    ...
                    <NavLink
                        ...
                        className={({ isActive }) => {
                            return (
                                ...
                                + (isActive || getParentPath() === item.path ? 'text-black border-blue-600 border-b-4' : 'text-slate-500')
                            )
                        }}>{item.title}</NavLink>
                )
                )}
    ...
    return (
        <div className="flex mr-10 gap-6">
            <div className="flex flex-col justify-center items-center">
                <BellIcon className="w-5 h-5 text-slate-400 fill-slate-400" />
                <span className="text-slate-400 text-xs">消息</span>
            </div>
            <div className="flex flex-col justify-center items-center">
                <ChatBubbleLeftRightIcon className="w-5 h-5 text-slate-400 fill-slate-400" />
                <span className="text-slate-400 text-xs">私信</span>
            </div>
            <div className="flex flex-col justify-center items-center">
                <UserIcon className="w-5 h-5 text-slate-400 fill-slate-400" />
                <span className="text-slate-400 text-xs">创作中心</span>
            </div>
              <div className="flex justify-center items-center">
                <img  src="https://picsum.photos/200/300" className="w-7 h-7 rounded-full"></img>
            </div>
        </div>
    )
}
    ...
    return (
        <div className="bg-white w-screen shadow h-13 overflow-hidden top-0 box-border sticky ">
            <div className="max-w-7xl mx-auto min-w-6xl my-0 flex justify-center">
                ...
                <div className={ 'w-full flex flex-col justify-between  min-w-max relative transition-all duration-300 ' +
                     (hide ? 'top-0' : '-top-13')}> 
